...
export async function loadTenancyAgreementArchive(
  ...
  const agreementsResult = await supabase
    ...
    .select(
      "id, tenancy_id, agreement_type, version_number, status, term_start_date, term_end_date, generated_at, signed_at, retention_until, pdf_url, tenant_name_snapshot, property_name_snapshot, room_name_snapshot",
    )
