...
export async function loadTenancyAgreementArchive(
  ...
  const agreementsResult = await supabase
    ...
    .select(
      "id, tenancy_id, term_type, agreement_type, version_number, status, term_start_date, term_end_date, generated_at, signed_at, admin_verified_at, admin_verified_by, admin_rejected_at, admin_rejected_by, admin_rejection_reason, replacement_agreement_id, is_correction, correction_reason, retention_until, pdf_url, tenant_name_snapshot, property_name_snapshot, room_name_snapshot, monthly_rent_snapshot",
    )
