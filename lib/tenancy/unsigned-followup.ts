...
export function neverSignedTenancies(agreements: AgreementArchiveItem[], today: string) {
  ...
  return [...groups.entries()].flatMap(([tenancyId, rows]) => {
    ...
    return [{ tenancyId, pending, expired, standby, overlap }];
  }).sort((a, b) => Number(b.expired) - Number(a.expired) ||
    (a.pending[0].property_name_snapshot ?? "").localeCompare(b.pending[0].property_name_snapshot ?? "") ||
    (a.pending[0].room_name_snapshot ?? "").localeCompare(b.pending[0].room_name_snapshot ?? "", undefined, { numeric: true }));
}
