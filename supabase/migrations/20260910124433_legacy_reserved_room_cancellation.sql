...
begin
 ...
 if a.status not in ('submitted','approved','pending_verification')
   or exists(select 1 from public.tenancies where tenant_application_id=a.id)
   or exists(select 1 from public.tenancies where room_id=a.room_id and status='active')
   or exists(select 1 from public.rooms where id=a.room_id and current_tenancy_id is not null)
   or (a.registration_mode<>'reservation' and not exists(select 1 from public.rooms where id=a.room_id and status='reserved'))
 then raise exception 'Reservation unavailable or room has an active tenancy'; end if;
...
create or replace function public.submit_public_reservation(p_application uuid,p_tenant uuid,p_payment_recorded boolean)
returns void language plpgsql security invoker set search_path='' as $$
declare a public.tenant_applications%rowtype; r public.rooms%rowtype;
begin
 select * into strict a from public.tenant_applications where id=p_application and tenant_id=p_tenant for update;
 select * into strict r from public.rooms where id=a.room_id for update;
 if a.registration_mode<>'reservation' or a.status not in ('draft','submitted')
 or r.status not in ('vacant','reserved') or r.current_tenancy_id is not null
 ...
 update public.tenant_applications set status='submitted',verification_status='pending_verification',
 payment_status=case when p_payment_recorded then 'pending_verification' else 'unpaid' end,
 submitted_at=coalesce(submitted_at,now()),updated_at=now() where id=a.id;
end;
$$;
revoke all on function public.submit_public_reservation(uuid,uuid,boolean) from public,anon,authenticated;
grant execute on function public.submit_public_reservation(uuid,uuid,boolean) to service_role;
