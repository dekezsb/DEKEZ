...
export default async function ReportsPage({ searchParams }: ReportsPageProps) {
  ...
  if (tab === "balance-sheet") {
    ...
    const addPosting = (accountId: string, id: string, date: string, reference: string, description: string, propertyId: string | null, netDebit: number, source: string, sourceHref?: string) => {
      ...
      base.push({...row, id, date, reference, description, propertyId, propertyName: propertyName(propertyId), amount: account.account_type === "asset" ? netDebit : -netDebit, source, sourceHref});
    };
    for (const line of journalLines) {
      const entry = journalEntryById.get(line.journal_entry_id);
      if (entry) addPosting(line.account_id, `journal-${line.id}`, entry.entry_date, entry.entry_number ?? entry.id, line.description ?? entry.description ?? "Journal", line.property_id, Number(line.debit) - Number(line.credit), "Posted journal", `/reports?tab=journal&month=${entry.entry_date.slice(0, 7)}&entry=${entry.id}#journal-${entry.id}`);
    }
    ...
    const makeSnapshot = (date: string, profit: typeof yearToDateReport) => {
      ...
      for (const bank of bankAccounts) {
        ...
        const bankCommon = {...bankDefinition, propertyId: null, propertyName: `Shared company bank · ${bank.name}`, shared: true, sourceHref: statement ? `/reports?tab=bank&month=${statement.period_end.slice(0, 7)}&statement=${statement.id}` : undefined};
            ...
            <>
              ...
              /></> : <Card id="bank-transactions">
                ...
                <CardContent className="space-y-4">
                  ...
                  {!reviewLines.length ? <div className="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-4 text-sm font-medium text-emerald-800">All {String(bankFlow) === "credit" ? "credits" : "debits"} have been matched or explained.</div> : null}
                </CardContent>
              </Card>}
            </>
