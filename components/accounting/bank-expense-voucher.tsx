...
export function BankExpenseVoucher({ lineId, amount, properties, accounts }: {
  lineId: string; amount: number; properties: { id: string; name: string }[]; accounts: { id: string; code: string; name: string }[];
}) {
  ...
  return <details className="rounded-lg border-2 border-blue-300 bg-blue-50" open>
    <summary className="cursor-pointer p-4 font-semibold text-blue-950">New payment voucher · enter expenses by line</summary>
    <form action={createBankExpenseVoucher} className="space-y-4 border-t border-blue-200 p-4">
      <p className="text-sm text-blue-900">For expenses not already recorded. Enter each property's share below. If bills already exist, use the receipt-matching section instead to avoid counting expenses twice.</p>
      ...
      {rows.map((row, index) => <fieldset key={row.id} className="grid gap-3 rounded-md border border-blue-200 bg-white p-3 md:grid-cols-2 xl:grid-cols-4">
        ...
        <label className="text-xs">Expense category<select className={field} value={row.accountId} onChange={(e) => update(row.id, "accountId", e.target.value)} required><option value="">Choose category</option>{accounts.map((a) => <option key={a.id} value={a.id}>{a.code} · {a.name}</option>)}</select></label>
      ...
      <label className="flex gap-2 text-sm"><input type="checkbox" name="confirmed" value="1" required />I have checked every line. These expenses have not already been recorded.</label>
