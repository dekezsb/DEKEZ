...
import { rankExistingPayments, type ExistingPayment, type StatementTransaction } from '@/lib/accounting/tenant-reconciliation';
  ...
  return <section className="space-y-3 rounded-lg border bg-white p-4" id="bank-transactions">
    ...
    <div className="max-h-[65vh] overflow-auto rounded border"><table className="w-full text-left text-xs"><thead className="sticky top-0 z-10 bg-slate-100"><tr>
      ...
      return <tr key={bank.id} className={`border-t ${linked?'bg-emerald-50':''}`}>
        <td className="min-w-64 p-2">{linked ? linked.tenant : <><strong className="block">{payment?.tenant??'No matching receipt selected'}</strong><button type="button" disabled={bank.used||locked||pending} className="text-blue-700 underline disabled:opacity-40" onClick={()=>setActiveBank(activeBank===bank.id?null:bank.id)}>Manual Match</button>{activeBank===bank.id?<select aria-label={`Existing payment for ${bank.reference || bank.id}`} className="mt-1 w-full rounded border p-1" value={selected[bank.id]??''} onChange={e=>setSelected(previous=>({...previous,[bank.id]:e.target.value}))} disabled={bank.used||locked||pending}><option value="">Select existing payment</option>{available.map(p=><option value={p.id} key={p.id} disabled={p.duplicate}>{p.tenant} · {p.property} {p.room} · {money(p.amount)} · {p.date} · {p.reference||p.receipt||p.id.slice(0,8)}</option>)}</select>:null}{ranked[0]&&!bank.used?<button type="button" disabled={pending||locked||bank.duplicate||ranked[0].payment.duplicate} className="mt-1 block text-blue-700 underline disabled:opacity-40" onClick={()=>setSelected(previous=>({...previous,[bank.id]:ranked[0].payment.id}))}>{selected[bank.id]===ranked[0].payment.id?'Suggested Match selected':'Select Suggested Match'} ({ranked[0].confidence})</button>:null}</>}</td>
        ...
        <td className="min-w-36 p-2">{linked ? canUnmatch?<button type="button" disabled={pending} className="text-red-700 underline" onClick={()=>run(bank.id,linked.id,true)}>Unmatch</button>:null : <button type="button" disabled={!selected[bank.id]||!payment||!same||bank.used||locked||pending||Boolean(payment.bankId)||payment.legacyMatched||duplicate} className="rounded bg-emerald-700 px-3 py-2 text-white disabled:opacity-40" onClick={()=>payment&&run(bank.id,payment.id)}>Reconcile</button>}
          {payment?<details className="mt-1"><summary>View Receipt</summary>{payment.tenant}<br/>{payment.receipt||'No receipt number assigned'}<br/>{money(payment.amount)} · {payment.date}<br/>Existing payment {payment.id}<br/>Tenant status (unchanged): {payment.tenantStatus}</details>:null}
