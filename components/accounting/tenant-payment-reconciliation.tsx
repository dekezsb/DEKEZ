...
import { bankPropertyRoomHint, rankExistingPayments, directReconciliationPayment, type ExistingPayment, type StatementTransaction } from '@/lib/accounting/tenant-reconciliation';
...
type ReceiptPreviewState = { url: string; title: string };
    ...
    >
      ...
      >
        <div className="flex items-start justify-between gap-4 border-b p-4">
          <div>
            <h3 className="font-semibold text-slate-950" id="receipt-preview-title">
              Receipt preview
            </h3>
            <p className="mt-1 text-sm text-slate-600">{preview.title}</p>
          </div>
          <button
        ...
        <div className="min-h-0 flex-1 bg-slate-100 p-3">
          <iframe
            className="h-[72vh] w-full rounded border bg-white"
            src={preview.url}
            title={preview.title}
          />
        </div>
...
export function TenantPaymentReconciliation({ payments, banks, locked, canUnmatch, statementMonth }: {payments:ExistingPayment[];banks:StatementTransaction[];locked:boolean;canUnmatch:boolean;statementMonth:string}) {
  ...
  const openPreview=(payment:ExistingPayment)=>{
    if(payment.slipUrl) setPreview({url:payment.slipUrl,title:'Payment slip · '+payment.tenant});
  };
  ...
  return <section className="space-y-3 rounded-lg border bg-white p-4" id="bank-transactions">
    ...
    <p className="rounded bg-blue-50 p-2 text-sm font-medium text-blue-950">Statement month: {statementMonth}. Only payments and receipts from this month are available to match.</p>
    ...
    <p className="text-sm text-slate-600">Clear matches with a receipt or slip are ready: Preview receipt, check the tenant and amount, then press Reconcile. Use Manual Match when proof is missing or the match needs review.</p>
    <p className="text-sm text-slate-600">Non-QR transfers can match the tenant name. No matching name or property / room reference means Manual Match only; amount and date alone never identify a tenant.</p>
    ...
    <div className="max-h-[65vh] overflow-auto rounded border"><table className="w-full text-left text-xs"><thead className="sticky top-0 z-10 bg-slate-100"><tr>
      ...
      const bankLocation=bankPropertyRoomHint(bank);
      ...
      return <tr key={bank.id} className={`border-t ${linked?'bg-emerald-50':''}`}>
        ...
        <td className="p-2">{payment?.property ?? bankLocation?.propertyCode ?? '—'}<br/>{payment?.room ?? (bankLocation ? `Room ${bankLocation.roomCode}` : 'No property / room reference')}</td><td className="p-2">{payment?.invoice||'—'}<br/>{payment?.receipt||'No receipt number'}</td>
        ...
        <td className="whitespace-nowrap p-2">{money(bank.amount)}<br/>{bank.date}</td><td className="max-w-64 p-2"><details><summary className="cursor-pointer">View Bank Transaction</summary>{bank.description}<br/>{bank.reference}<br/>{bank.id}</details><span className="block">{bank.reference||bank.description.slice(0,55)}</span>{bankLocation?<span className="mt-1 block font-medium text-slate-700">Property / room: {bankLocation.propertyCode} · Room {bankLocation.roomCode}</span>:<span className="mt-1 block text-slate-500">No property / room reference in bank transaction</span>}</td>
        ...
        <td className="min-w-36 p-2">{linked ? canUnmatch?<button type="button" disabled={pending} className="text-red-700 underline" onClick={()=>run(bank.id,linked.id,true)}>Unmatch</button>:null : <button type="button" disabled={!chosenPayment||!payment||!same||bank.used||locked||pending||Boolean(payment.bankId)||payment.legacyMatched||duplicate} className="rounded bg-emerald-700 px-3 py-2 text-white disabled:opacity-40" onClick={()=>chosenPayment&&run(bank.id,chosenPayment.id)}>Reconcile</button>}
          {payment?<details className="mt-1"><summary>Receipt details</summary><div className="mt-2 space-y-1 rounded border bg-white p-2"><strong>{payment.tenant}</strong><p>{payment.property} / {payment.room}</p><p>Invoice: {payment.invoice||'—'}<br/>Receipt: {payment.receipt||'No receipt number assigned'}</p><p>Receipt amount: {money(payment.amount)}<br/>Bank amount: {money(bank.amount)}</p><p>Payment date: {payment.date}<br/>Bank date: {bank.date}</p><p>Payment reference: {payment.reference||'—'}<br/>Bank reference: {bank.reference||bank.description}</p><p>Existing payment {payment.id}<br/>Tenant status (unchanged): {payment.tenantStatus}</p></div></details>:null}
          {payment?.slipUrl?<button type="button" className="block text-blue-700 underline" onClick={()=>openPreview(payment)}>Preview receipt</button>:null}
